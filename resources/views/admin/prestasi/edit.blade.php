...
<form action="{{ route('prestasi.update', $prestasi->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
        <div class="col">
            <div class="form-group">
                ...
                @if ($prestasi->foto)
                    <img src="{{ asset('storage/' . $prestasi->foto) }}" alt="Foto" class="img-thumbnail img-preview mt-2" width="200">
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col">
            <div class="form-group">
                <label for="judul">Nama Prestasi<span class="text-danger">*</span></label>
                <input type="text" class="form-control @error('judul') is-invalid @enderror" id="judul" name="judul" required value="{{ old('judul', $prestasi->judul) }}">
                @error('judul')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
    ...
    <div class="row mt-3">
        <div class="col">
            <div class="form-group">
                ...
                <textarea name="isi" id="summernote" rows="5" class="form-control @error('isi') is-invalid @enderror" required>{{ old('isi', $prestasi->isi) }}</textarea>
                @error('isi')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
